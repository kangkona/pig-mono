...
_BY_BARE: dict[str, tuple[int, float, float]] | None = None


def _by_bare() -> dict[str, tuple[int, float, float]]:
    global _BY_BARE
    if _BY_BARE is None:
        index: dict[str, tuple[int, float, float]] = {}
...
def get_model_info(model: str | None) -> dict[str, float] | None:
    """Return ``{context_window, input_cost, output_cost}`` for a model, or None.
    ...
    if entry is None:
        return None
    context_window, input_cost, output_cost = entry
    return {
