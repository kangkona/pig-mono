...
| Compaction | Supported contract | Manual, threshold, and overflow compactions have distinct reasons and durable before/after checkpoints in session metadata. Semantic summarization is branch-local, bounded, provider-completion-gated, and keeps the exact recent tail; a failed summary or atomic save leaves the existing tree and durable file unchanged. Branch-summary provider usage and structural compaction remain separate categories. Canonical assistant `tool_calls`, tool `tool_call_id` values, and activation anchors survive save/load and compaction; provider-specific auxiliary envelope fields beyond that canonical contract are not claimed. |
