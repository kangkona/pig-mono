...
def test_main_uses_env_session_dir_for_session_lookup(mock_agent_class, mock_llm_class, tmp_path):
    ...
    custom_session_dir = tmp_path / "custom-sessions"
    source_session = custom_session_dir / "source-1234.jsonl"
    custom_session_dir.mkdir(parents=True)
    source_session.write_text("{}\n")

    with (
        patch.dict("os.environ", {"PIG_CODING_AGENT_SESSION_DIR": str(custom_session_dir)}),
        patch("pig_coding_agent.cli.console"),
        patch("pig_agent_core.SessionManager") as mock_session_manager_class,
    ):
        mock_session_manager = Mock()
...
def test_main_uses_global_config_session_dir_for_lookup(
    ...
    custom_session_dir = tmp_path / "global-sessions"
    source_session = custom_session_dir / "source-1234.jsonl"
    custom_session_dir.mkdir(parents=True)
    source_session.write_text("{}\n")
    ...
    ((fake_home / ".agents") / "config.json").write_text(
        json.dumps({"session_dir": str(custom_session_dir)})
    )
